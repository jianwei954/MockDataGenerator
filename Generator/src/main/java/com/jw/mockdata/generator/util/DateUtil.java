...
public final class DateUtil
{
    ...
    public static String formatGMTDate(String date,String format) throws Exception
    {
...
//            throw new Exception("日期数据解析失败", e);
        }
        return da;
    }
    ...
    public static Date formatDateStr(Object datestr)
        throws Exception
    {
        if (null == datestr||datestr.toString().equals("(null)"))
            return null;
        Date trydate = null;
        ...
        try
        {
            ...
            trydate = format.parse(date.toString());
        }
        catch (Exception e)
        {
        }
        ...
        if (Pattern.matches(date_format_3_2, datestr.toString()))
        {
            try
            {
                ...
                int d1 = Integer.parseInt(splites[0]);
                int d2 = Integer.parseInt(splites[1]);
                int d3 = Integer.parseInt(splites[2]);
                if (d1 <= 12)
                {
                    if (d2 <= 12)
                    {
        ...
        try
        {
            ...
            for (int i = 0; i < numbers.size(); i++)
            {
                ...
                if ((prespan.endsWith("下午") || prespan.endsWith("pm")) || (afterspan.endsWith("下午") || afterspan.endsWith("pm")))
                    isPM = true;
                else if ((prespan.endsWith("上午") || prespan.endsWith("am")) || (afterspan.endsWith("上午") || afterspan.endsWith("am")))
                    isPM = false;
                ...
                for (int i1 = 0; i1 < years.size(); i1++)
                {
                    ...
                    else
                    {
                        for (int i2 = 0; i2 < months.size(); i2++)
                        {
                            ...
                            if (days.size() == 0)
                            {
                                d = ParseOtherDate(others, month.getIndex(), 2, isPM, syear, smonth, "01", "00", "00", "00");
                                if (d != null)
                                    return d;
                            }
                            else
                            {
                                for (int i3 = 0; i3 < days.size(); i3++)
                                {
                                    ...
                                    if (hours.size() == 0)
                                    {
                                        d = ParseOtherDate(others, day.getIndex(), 3, isPM, syear, smonth, sday, "00", "00", "00");
                                        if (d != null)
                                            return d;
                                    }
                                    else
                                    {
                                        for (int i4 = 0; i4 < hours.size(); i4++)
                                        {
                                            ...
                                            {
                                                d =
                                                    ParseOtherDate(others, hour.getIndex(), 4, isPM, syear, smonth, sday, shour, "00", "00");
                                                if (d != null)
                                                    return d;
                                            }
                                            else
            ...
            if (d == null)
            {
//                Logger.error(DateUtil.class.getName()+datestr.toString());
//                throw new Exception("时间类型转换错误");
                return null;
            }
        }
        catch (Exception e)
    ...
    private static Date ParseOtherDate(ArrayList<NumberItem> others, int afterindex, int inisize, Boolean isPM, String... snumber)
    {
        if (others.size() == 0)
        {
    ...
    private static Date ParseDate(String s, String pattern)
    {
        ...
        catch (ParseException e)
        {
            ...
            {
                log.error("日期数据解析失败, datestr=" + s,e);
            }
        }
        return da;
    }
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if (Pattern.matches(date_format_1, dateString))
            {
                ...
                else if (dateString.indexOf("下午") != -1 || dateString.indexOf("pm") != -1 || dateString.indexOf("PM") != -1)
                {
        ...
        {
            log.error("日期数据解析失败, datestr=" + datestr,e);
            throw e;
        }
        return d;
    }
     ...
     * @return
     */
    public static Date getDate2Hour(Date date)
    {
        Calendar cal = Calendar.getInstance(Locale.CHINA);
        cal.setTime(date);
    ...
    /**
     * 比较时间大小，如果返回值>0表示d1>d2,返回值 = 0 表示 d1 = d2 
     * @param d1
     * @param d2
     * @return
     */
    ...
    /**
     * 比较日期大小，是否同一天，如果返回值>0表示d1>d2,返回值 = 0 表示 d1 = d2
     * @param date1
     * @param date2
     * @return
     */
    ...
    public static String getDay(Date date) {
        ...
        time.setTime(date);
        DecimalFormat df = new DecimalFormat("00");
        return df.format(time.get(Calendar.DAY_OF_MONTH));
    }
